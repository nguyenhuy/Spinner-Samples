...
class ServerConfiguration {
	...
	func loadFromFileAtPath(path: String) -> Bool {
		...
		do {
            newConfiguration = try PropertyListSerialization
                .propertyList(with: fileStream, options: .mutableContainers, format: nil) as! [String: AnyObject]
		}
		catch {
		...
		if let value = newConfiguration[SettingsKey.IPv4.rawValue] as? [NSObject: AnyObject] {
            ...
            IPv4Dictionary
                .removeValue(forKey: SettingsKey.Pool.rawValue as NSObject)
            newConfiguration[SettingsKey.IPv4.rawValue] = IPv4Dictionary as AnyObject
		}

        if !newConfiguration.keys
            .contains(where: { $0 == SettingsKey.DNS.rawValue }) {
	...
	class func copyDNSConfigurationFromSystem() -> ([String], [String]) {
		...
		if let globalDNS = SCDynamicStoreCopyValue(nil, globalDNSKey),
