...
class ServerTunnelConnection: Connection {
	...
	func open() -> Bool {
        ...
        guard setupVirtualInterface(address: address) else {
            ...
            ServerTunnel.configuration.addressPool?
                .deallocateAddress(addrString: address)
...
//        var socketAddressControl = sockaddr_ctl(sc_len: UInt8(MemoryLayout.size(ofValue: sockaddr_ctl.self)), sc_family: UInt8(AF_SYSTEM), ss_sysaddr: UInt16(AF_SYS_CONTROL), sc_id: controlIdentifier, sc_unit: 0, sc_reserved: (0, 0, 0, 0, 0))
//
...
//        if !errorString.isEmpty, connectResult < 0 {
...
//		}
	...
	func getTUNInterfaceName(utunSocket: Int32) -> String? {
        ...
        let errorString = String(cString: strerror(errno))
        if !errorString.isEmpty, resultCode < 0 {
	...
	func setupVirtualInterface(address: String) -> Bool {
        ...
        guard let interfaceName = getTUNInterfaceName(
            utunSocket: utunSocket
        ), utunSocket >= 0 &&
	...
	func readPackets() {
        ...
        let utunSocket: UInt32 = source.handle
		...
		repeat {
			...
			guard readCount > 0 || errno == EAGAIN else {
                let errorString = String(cString: strerror(errno))
                if !errorString.isEmpty, readCount < 0 {
	...
	func startTunnelSource(utunSocket: Int32) {
		...
		guard let newSource = dispatch_source_create(_dispatch_source_type_read, UInt(utunSocket), 0, dispatch_get_main_queue()) else { return }
    ...
    override func closeConnection(_ direction: TunnelConnectionCloseDirection) {
		...
		if currentCloseDirection == .all {
			...
			if tunnelAddress != nil {
                ServerTunnel.configuration.addressPool?
                    .deallocateAddress(addrString: tunnelAddress!)
			}
			utunName = nil
		}
	}
    ...
    override func sendPackets(_ packets: [Data], protocols: [NSNumber]) {
        ...
        for (index, packet) in packets.enumerated() {
            ...
            let buffer = UnsafeMutableRawPointer(bitPattern: packet.count)
            var iovecList = [
                iovec(
                    iov_base: &protocolNumber,
                    iov_len: MemoryLayout.size(ofValue: protocolNumber)
                ),
            ...
            else if writeCount < packet.count + MemoryLayout.size(
                ofValue: protocolNumber
            ) {
