...
class UDPServerConnection: Connection {
    ...
    func getEndpointFromSocketAddress(socketAddressPointer: UnsafePointer<sockaddr>) -> (host: String, port: Int)? {
        ...
        // HERE
//        let socketAddress = UnsafePointer<sockaddr>(
//            socketAddressPointer
//        ).pointee
//
//		switch Int32(socketAddress.sa_family) {
//			case AF_INET:
//            var socketAddressInet = UnsafePointer<sockaddr_in>(
//                socketAddressPointer
//            ).pointee
...
//
//        guard let newResponseSource = dispatch_source_create(_dispatch_source_type_read, UInt(newSocket), 0, dispatch_get_main_queue()) else {
//			close(newSocket)
//			return false
//		}
//
...
//            let UDPSocket = newResponseSource.handle
//			close(UDPSocket)
//		}
//            
...
//                MemoryLayout.size(ofValue: sockaddr_storage.self)
//            )
...
//                recvfrom(
//                    UDPSocket,
//                    UnsafeMutablePointer<Void>(mutating: response),
//                    response.count,
//                    0,
...
//				if let errorString = String(cString: strerror(errno)) {
