...
class ServerTunnel: Tunnel, TunnelDelegate, StreamDelegate {
		...
		switch tunnelLayer {
			case .app:
				...
				switch flowKind {
                    ...
                    guard newConnection
                        .open(host: host, port: port.intValue) else {
    ...
    override func closeTunnel() {
        ...
        if let stream = readStream {
            ...
            let socketData = CFReadStreamCopyProperty(
                stream,
    ...
    override func handleMessage(_ commandType: TunnelCommand, properties: [String: AnyObject], connection: Connection?) -> Bool {
		switch commandType {
            ...
            let messageProperties = createMessagePropertiesForConnection(
                ...
                extraProperties: [TunnelMessageKey.Configuration.rawValue: personalized as AnyObject]
            )
    ...
    func tunnelDidSendConfiguration(
        _ targetTunnel: Tunnel,
